...
#region DIbuja el titulo
	...
	if !(global.tarea_completada)
	{
		...
		switch(os_get_language()) {
			case "es": draw_text_outline("MUEVA LA AGUJA AL PUNTO",posX,posY,borde,1,1,color) break;
			case "pt": draw_text_outline("MOVA A AGULHA PARA O PONTO",posX,posY,borde,1,1,color) break;
			case "it": draw_text_outline("SPOSTA L'AGO VERSO IL PUNTO",posX,posY,borde,1,1,color) break;
			case "ru": draw_set_font(fnt_normal_ru) draw_text_outline("ПОДВОДИТЕ ИГЛУ В ТОЧКУ",posX,posY,borde,1,1,color) break;
			default: draw_text_outline("MOVE THE NEEDLE TO THE POINT",posX,posY,borde,1,1,color) break;
		}
	}	
	else 
	{
...
#region Dibuja las barras
	var barra_ancho = 100
	var barra_alto = 300
	var color_fondo = make_color_rgb(53,53,53)
	var color_valor = make_color_rgb(92,255,87)
	var posY = (cam_y+416)+150
	var posX = (cam_x+1120)-50
	
	// Barra 
	draw_healthbar(posX, posY,posX+barra_ancho, posY-barra_alto, tiempo, color_fondo, color_valor, color_valor, 2, true, true)
#endregion	
