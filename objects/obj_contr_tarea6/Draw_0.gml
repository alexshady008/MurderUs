...
#region DIbuja el titulo
	...
	if !(global.tarea_completada)
	{
		...
		switch(os_get_language()) {
			case "es": draw_text_outline("ENCIENDE TODOS LOS INTERRUPTORES",posX,posY,borde,1,1,color) break;
			case "pt": draw_text_outline("LIGUE TODOS OS INTERRUPTORES",posX,posY,borde,1,1,color) break;
			case "it": draw_text_outline("ACCENDI TUTTI GLI INTERRUTTORI",posX,posY,borde,1,1,color) break;
			case "ru": draw_set_font(fnt_normal_ru) draw_text_outline("ВКЛЮЧИТЕ ВСЕ ВЫКЛЮЧАТЕЛИ",posX,posY,borde,1,1,color) break;
			default: draw_text_outline("TURN ALL THE SWITCHES ON",posX,posY,borde,1,1,color) break;
		}
	}	
	else 
	{
