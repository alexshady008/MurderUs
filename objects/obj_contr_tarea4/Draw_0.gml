...
#region DIbuja el titulo
	...
	if !(global.tarea_completada)
	{
		...
		switch(os_get_language()) {
			case "es": draw_text_outline("CONECTAR LOS PUNTOS DEL MISMO COLOR",posX,posY,borde,1,1,color) break;
			case "pt": draw_text_outline("CONECTE OS PONTOS DA MESMA COR",posX,posY,borde,1,1,color) break;
			case "it": draw_text_outline("CONNETTI PUNTI DELLO STESSO COLORE",posX,posY,borde,1,1,color) break;
			case "ru": draw_set_font(fnt_normal_ru) draw_text_outline("СОЕДИНИТЕ ТОЧКИ ОДНОГО ЦВЕТА",posX,posY,borde,1,1,color) break;
			default: draw_text_outline("CONNECT THE POINTS OF THE SAME COLOR",posX,posY,borde,1,1,color) break;
		}
	}	
	else 
	{
...
#region Dibuja el campo y el valor
	if (btn_presionado!=noone) draw_line_width_color(btn_presionado.x,btn_presionado.y,mouse_x,mouse_y,50,color_linea,color_linea)
#endregion